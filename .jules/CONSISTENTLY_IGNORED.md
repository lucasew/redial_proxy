...
**- Pattern:** Extracting simple logic (like flag parsing or environment variable setup in main.go) into isolated functions or packages when it's only used once.
**- Justification:** Violates the Rule of Three. Abstracting single-use code adds unnecessary indirection and complexity without improving reusability. NOTE: This does NOT apply to centralized error reporting, which is mandatory.
**- Files Affected:** `cmd/redial_proxy/main.go`, `internal/config/*`
...
**- Pattern:** Making localized fixes (like changing `time.After` to `time.NewTimer`, or `fmt.Sprintf` to `strconv.Itoa`) without fixing existing violations of the same rule in the same file or surrounding context (e.g., leaving `os.Exit(1)` or unhandled `slog` errors).
...
**- Files Affected:** Any file with repeated patterns, e.g., `cmd/redial_proxy/main.go`, `internal/dialer/dialer.go`

## IGNORE: Misconfigured golangci-lint

**- Pattern:** Removing `golangci-lint` from `[tools]` in `mise.toml`, changing its version to `latest`, or attempting to install it via `go install` in tasks.
**- Justification:** `golangci-lint` must be managed via `mise` tools and pinned to a specific version (e.g., `2.8.0`) to ensure stability and proper integration with the development workflow.
**- Files Affected:** `mise.toml`
